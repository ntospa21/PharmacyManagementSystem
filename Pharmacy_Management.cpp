...
void medicineType::take_order() // function to take_order
{
	...
	else
	{
		for (i = 0; i < temp->x; i++)
		{
			...
			cout << "The amount You need to pay is: " << temp->amount[i] << " EUR" << endl;
			system("PAUSE");
		}
		cout << "===========================================================================" << endl;
			...
			if (found)
			{
				...
				else
				{
					for (i = 0; i < temp->x; i++)
					{
						...
						cin >> temp->quantity[i];
						temp->amount[i] = temp->quantity[i] * temp->Medicine[temp->menu2[i] - 1];
						cout << "The amount You need to pay After Modify  is: " << temp->amount[i] << " EUR" << endl;
						system("PAUSE");
					}
					temp = temp->next;
					system("cls");
